defmodule ZarynWeb.GraphQLSchemaTest do
  use ZarynCase
  use ZarynWeb.ConnCase
  use ZarynWeb.GraphQLSubscriptionCase
  ...
  alias Zaryn.P2P
  alias Zaryn.P2P.Message.GetLastTransaction
  ...
  describe "query: balance" do
    test "should retrieve the uco balance of an address", %{conn: conn} do
      ...
      conn =
        post(conn, "/api", %{
          "query" => "query { balance(address: \"#{Base.encode16(addr)}\") { zaryn } }"
        })

      assert %{"data" => %{"balance" => %{"zaryn" => 2.18}}} = json_response(conn, 200)
    end
