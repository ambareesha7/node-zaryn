defmodule Zaryn.P2P.Message do
  @moduledoc """
  ...
  def encode(%Balance{zaryn: zaryn_balance, nft: nft_balances}) do
    nft_balances_binary =
      nft_balances
    ...
    <<249::8, zaryn_balance::float, map_size(nft_balances)::16, nft_balances_binary::binary>>
  end
  ...
  def decode(<<249::8, zaryn_balance::float, nb_nft_balances::16, rest::bitstring>>) do
  ...
  def process(%NewTransaction{transaction: tx}) do
    ...
    case Zaryn.send_new_transaction(tx) do
      :ok ->
  ...
  def process(%GetBalance{address: address}) do
    %{zaryn: zaryn, nft: nft} = Account.get_balance(address)
