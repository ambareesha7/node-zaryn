defmodule Zaryn.BeaconChain.SlotTimer do
  @moduledoc """
  ...
  defp get_interval do
    [{_, interval}] = :ets.lookup(:zaryn_slot_timer, :interval)
    interval
  end
  ...
  def init(opts) do
    ...
    :ets.new(:zaryn_slot_timer, [:named_table, :public, read_concurrency: true])
    :ets.insert(:zaryn_slot_timer, {:interval, interval})
  ...
  def handle_cast({:new_conf, conf}, state) do
    case Keyword.get(conf, :interval) do
      ...
      new_interval ->
        :ets.insert(:zaryn_slot_timer, {:interval, new_interval})
