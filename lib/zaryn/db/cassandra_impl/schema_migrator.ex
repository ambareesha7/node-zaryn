defmodule Zaryn.DB.CassandraImpl.SchemaMigrator do
  @moduledoc false
  ...
  defp create_keyspace do
    """
      CREATE KEYSPACE IF NOT EXISTS zaryn WITH replication = {
  ...
  defp create_migration_table do
    """
    CREATE TABLE IF NOT EXISTS zaryn.schema_migrations(
      version INT,
  ...
  defp get_migrated_versions do
    Producer.add_query("SELECT version FROM zaryn.schema_migrations")
  end
  ...
  defp migration_path do
    Application.app_dir(:zaryn, "priv/migrations")
  end
  ...
  defp register_migrated_versions(versions) do
    query = "INSERT INTO zaryn.schema_migrations(version, updated_at) VALUES(?, ?)"
