defmodule Zaryn.Crypto.NodeKeystore.TPMImpl do
  @moduledoc false

  alias Zaryn.Crypto
  alias Zaryn.Crypto.ID
  alias Zaryn.Crypto.NodeKeystore

  alias Zaryn.TransactionChain

  alias Zaryn.Utils.PortHandler
  ...
  @impl GenServer
  def init(_) do
    tpm_program = Application.app_dir(:zaryn, "priv/c_dist/tpm_port")
  ...
  defp sign(port_handler, index, data) do
    ...
    :telemetry.execute([:zaryn, :crypto, :tpm_sign], %{
