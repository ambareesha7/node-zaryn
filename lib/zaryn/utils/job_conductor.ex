defmodule Zaryn.JobConductor do
  @moduledoc ~S"""
  ...
  ## Example
      ...
      iex> end
      iex>
      iex> {:ok, _} = Zaryn.JobConductor.start_link [name: Zaryn.JobConductor]
      iex>
      iex> spawn(fn -> Zaryn.JobConductor.conduct f, [1] end)
      iex> spawn(fn -> Zaryn.JobConductor.conduct f, [2] end)
      iex> Process.sleep(5) # let spawned calls some time to spawn
      iex>
      iex> Zaryn.JobConductor.conduct f, [3]
      {:ok, :done}
      iex>
  ...
  ## Example
      ...
      iex> {:ok, c} = Zaryn.JobConductor.start_link []
      iex>
      iex> Zaryn.JobConductor.conduct(fn -> :done end, [], c)
      {:ok, :done}
      iex>
      iex> Zaryn.JobConductor.conduct(fn -> raise "exception" end, [], c)
      {:rescued, %RuntimeError{message: "exception"}}
      iex>
      iex> Zaryn.JobConductor.conduct(fn -> throw :garbage end, [], c)
      {:caught, :garbage}
  """
  ...
  @doc ~S"""
  Starts a `Zaryn.JobConductor` process linked to the current process.
