defmodule Zaryn.Testnet do
  @moduledoc """
  Zaryn Testnet Generator
  """
  ...
  @p2p_port Application.compile_env!(:zaryn, Zaryn.P2P.Endpoint)[:port]
  @web_port Application.compile_env!(:zaryn, ZarynWeb.Endpoint)[:http][:port]
        ...
        version: "3.9",
          ...
          services: %{
            "node1" => %{
              ...
              environment: %{
                "ZARYN_CRYPTO_SEED" => "seed1",
                "ZARYN_MUT_DIR" => "/opt/data",
                "ZARYN_P2P_SEEDS" => "1.2.3.2:#{@p2p_port}:009D421F8ACC11F7E25EC515732ECB7E3E38109118EC409701930D7A892E913428:tcp",
                "ZARYN_STATIC_IP" => "1.2.3.2"},
              image: "i",
              networks: %{:net => %{ipv4_address: "1.2.3.2"}},
              volumes: [".data1:/opt/data"]},
            "node2" => %{
              command: ["/wait.sh", "http://node1:#{@web_port}/up", "./bin/zaryn_node", "foreground"],
              depends_on: ["node1"],
              environment: %{
                "ZARYN_CRYPTO_SEED" => "seed2",
                "ZARYN_MUT_DIR" => "/opt/data",
                "ZARYN_P2P_SEEDS" => "1.2.3.2:#{@p2p_port}:009D421F8ACC11F7E25EC515732ECB7E3E38109118EC409701930D7A892E913428:tcp",
                "ZARYN_STATIC_IP" => "1.2.3.3"},
              image: "i",
  ...
  defp to_node({[seed], 1}, persist?, src, image, ip) do
    {"node1",
     %{
       ...
       image: image,
       environment: %{
         "ZARYN_CRYPTO_SEED" => seed,
         "ZARYN_P2P_SEEDS" => seeder(ip.(1 + 1), seed),
         "ZARYN_STATIC_IP" => ip.(1 + 1)
       },
  ...
  defp to_node({[seed | seeds], n}, persist?, src, image, ip) do
    ...
    {"node#{n}",
     %{
       ...
       environment: %{
         "ZARYN_CRYPTO_SEED" => seed,
         "ZARYN_P2P_SEEDS" => seeders,
         "ZARYN_STATIC_IP" => ip.(n + 1)
       },
