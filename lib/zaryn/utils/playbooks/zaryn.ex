defmodule Zaryn.Playbook.ZARYN do
  @moduledoc """
  Play and verify ZARYN ledger.
  ...
  def transfer(_from, _to, _amount) do
    ...
    tx = %{
      ...
      "data" => %{
        "ledger" => %{
          "zaryn" => %{
            "transfers" => [
              %{"to" => Base.encode16(zaryn_to), "amount" => 12.34}
            ]
          }
        }
      }
    ...
    query = ~s|query {balance(address: "#{address}"){zaryn}}|
    WebClient.with_connection(host, port, &WebClient.query(&1, query))
  end

  def play!(nodes, opts) do
    Logger.info("Play ZARYN transactions on #{inspect(nodes)} with #{inspect(opts)}")
    port = Application.get_env(:zaryn, ZarynWeb.Endpoint)[:http][:port]
