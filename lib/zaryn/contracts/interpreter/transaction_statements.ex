defmodule Zaryn.Contracts.Interpreter.TransactionStatements do
  @moduledoc false
  ...
  @doc """
  Add a ZARYN transfer

  ## Examples

      iex> TransactionStatements.add_zaryn_transfer(%Contract{}, [{"to", "22368B50D3B2976787CFCC27508A8E8C67483219825F998FC9D6908D54D0FE10"}, {"amount", 10.04}])
      %Contract{
  ...
  """
  @spec add_zaryn_transfer(Contract.t(), list()) :: Contract.t()
  def add_zaryn_transfer(contract = %Contract{}, [{"to", to}, {"amount", amount}])
      when is_binary(to) and is_float(amount) do
    update_in(
      contract,
      access_path([:next_transaction, :data, :ledger, :zaryn, :transfers]),
      &[%ZARYNLedger.Transfer{to: decode_binary(to), amount: amount} | &1]
    )
  end
