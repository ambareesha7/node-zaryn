...
config :zaryn, :src_dir, File.cwd!()

config :zaryn, :mut_dir, "data"
...
config :zaryn, Zaryn.Crypto.NodeKeystore.SoftwareImpl, seed: System.get_env("ZARYN_CRYPTO_SEED")

config :zaryn, Zaryn.DB, Zaryn.DB.CassandraImpl

config :zaryn, Zaryn.Bootstrap.NetworkInit,
  genesis_seed:
...
config :zaryn, Zaryn.SelfRepair.Sync, last_sync_file: "p2p/last_sync"

# Configure the endpoint
config :zaryn, ZarynWeb.Endpoint,
  secret_key_base: "9yDOgbsd+CDHJ7fNk5fDB71o6FFJLy+ZSQXvOB8ibo78cCYC9mAhCvfRJzH+wkIG",
  render_errors: [view: ZarynWeb.ErrorView, accepts: ~w(json)],
  pubsub_server: ZarynWeb.PubSub,
  live_view: [
    signing_salt: "clEKFN96",
...
config :zaryn, Zaryn.Mining, timeout: 3_000

config :zaryn, Zaryn.OracleChain,
  services: [
    zaryn: Zaryn.OracleChain.Services.ZARYNPrice
  ]

config :zaryn, Zaryn.OracleChain.Services.ZARYNPrice,
  provider: Zaryn.OracleChain.Services.ZARYNPrice.Providers.Coingecko
