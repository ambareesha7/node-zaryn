...
then
    ...
    cp _build/prod/rel/zaryn_node/releases/$VERSION/zaryn_node.tar.gz $INSTALL_DIR/releases/$VERSION

    echo "Run the upgrade"
    $INSTALL_DIR/bin/zaryn_node upgrade ${VERSION}
else
    # Build the releases
    ...
    tar zxvf _build/prod/rel/zaryn_node/releases/$VERSION/zaryn_node.tar.gz -C $INSTALL_DIR
